...
const loadGames = async () => {
  ...
  const { data, meta } = await gamesStore.getUserGames( ++pagination.page )
  userGames.value = pagination.page > 1 ? userGames.value.concat(data) : data
...
<template>
  <div v-if="userGames.length">
    <masonry-wall :items="userGames" :gap="16">
